...
> **סטטוס:** שלב 3 מתוך 4 — עריכה אוטומטית עם Claude. ראו [מפת דרכים](#מפת-דרכים).
...
|---|---|---|
| `ANTHROPIC_API_KEY` | מפתח Claude API. נשמר בשרת בלבד ולא נשלח לדפדפן. בלעדיו האפליקציה עובדת עם העריכה האוטומטית מבוססת ההיסטוגרמה בלבד | — |
| `CLAUDE_MODEL` | המודל לניתוח תמונות ולבקשות בשפה חופשית | `claude-sonnet-5` |
| `APP_PASSWORD` | אופציונלי, **מומלץ מאוד באתר ציבורי**: סיסמה שנדרשת לשימוש ב-Claude, כדי שזרים לא ישתמשו במפתח שלך. הדפדפן ישאל אותה פעם אחת | — |
...
כשתמונה נטענת:
1. **מיד** מוחלת עריכה אוטומטית שמחושבת מההיסטוגרמה (חשיפה, Highlights/Shadows, נקודות לבן/שחור, ניגודיות,
   איזון לבן זהיר, ווייברנס, וחידוד והפחתת רעש לפי ה-ISO). היא עובדת גם בלי מפתח API.
2. **ברקע** נשלחת ל-Claude תמונה מוקטנת (1024px), יחד עם נתוני הצילום, סטטיסטיקות הבהירות וההצעה של ההיסטוגרמה.
   Claude מזהה את סוג התמונה (נוף, פורטרט, לילה, אוכל…) ומחזיר JSON עם כל הפרמטרים, שנבדק ומוגבל לטווחים בשרת.
   כשהתשובה מגיעה (בדרך כלל 10–30 שניות) היא מוחלת. אם כבר התחלת לערוך, היא מחכה בפאנל עם כפתור **Apply**.
3. Claude יכול גם להציע **יישור** ו**חיתוך**. ההצעה מופיעה ב-**Straighten & crop** ומוחלת רק אם לוחצים **Apply**.
   גם ההיסטוגרמה מזהה אופק עקום ומציעה יישור.

**Reset / Reset to auto** מחזירים לעריכה האוטומטית. כל שינוי אוטומטי נרשם ב-Undo.

**בקשה בשפה חופשית:** בפאנל **Auto edit** כותבים מה רוצים, בכל שפה ("תעשה את זה יותר חמים וקולנועי").
Claude רואה את התמונה הנוכחית ואת הפרמטרים, ומחזיר ערכים חדשים עם הסבר קצר. גם זה נרשם ב-Undo.
...
4. תחת **Environment Variables** הוסיפו `ANTHROPIC_API_KEY`, ו-`APP_PASSWORD` (כדי שרק אתם תוכלו להשתמש ב-Claude דרך האתר).
   אחרי שינוי משתנים צריך **Redeploy** (Deployments ← ⋯ ← Redeploy).
5. **Deploy**.
...
npm test            # בדיקות יחידה (vitest): סכמה, עקומות, מיפוי טונים, זום, Undo/Redo,
                    # אלגוריתם ההיסטוגרמה, זיהוי אופק, ונתיבי ה-API מול Claude מדומה
...
### בדיקת Claude בלי מפתח

`e2e/fake-anthropic.mjs` מדמה את ה-API של Anthropic ומחזיר עריכה קבועה:

```bash
node e2e/fake-anthropic.mjs &
...
client/src/analysis/  סטטיסטיקות, עריכה אוטומטית מההיסטוגרמה, זיהוי אופק (Hough)
client/src/auto/      תזמור: עריכת היסטוגרמה → Claude → בקשות חופשיות
server/src/claude/    הסכמה ש-Claude ממלא (structured outputs), הפרומפטים, והקריאה ל-API
...
- [x] **שלב 3** — עריכה אוטומטית: אלגוריתם היסטוגרמה + ניתוח Claude vision, בקשות בשפה חופשית, הצעות יישור וחיתוך
